<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
    ...
    class Restro_Owner extends CI_Controller
    {
        ...
        public function setup_restro_seating_hours(){
            ...
            $data['errors'] = 'Invalid restaurant';
            }*/
                ...
                for($i=0; $i<3; $i++) {
                    ...
                    foreach($weekdays as $weekday){
                        $info = array_merge($info, array(                    
                            $weekday."_from"=>$this->input->post("seating_info_".$weekday."_from")[$i],
                            $weekday."_to"=>$this->input->post("seating_info_".$weekday."_to")[$i],
                            $weekday."_max_cover"=>$this->input->post("seating_info_".$weekday."_max_cover")[$i],
                            $weekday."_largest_party_size"=>$this->input->post("seating_info_".$weekday."_largest_party_size")[$i],
                            $weekday."_booking_limit"=>$this->input->post("seating_info_".$weekday."_booking_limit")[$i],
                            $weekday."_cover_count"=>$this->input->post("seating_info_".$weekday."_cover_count")[$i],
                            $weekday."_point"=>$this->input->post("seating_info_".$weekday."_point")[$i],
                            $weekday."_deposit"=>$this->input->post("seating_info_".$weekday."_deposit")[$i]
                        ));
                    }
                    ...
                    $this->RestroSeatingHourModel->create($info);
                }

                redirect('/setup_restro_seating_hours/'.$restro_id.'/'.$location_id);
                //$this->load->view("Restaurant_Owner/setup_restro_seating_hours", array("test_data"=>'asdf'));
            }
            $this->load->view("Restaurant_Owner/setup_restro_seating_hours", $data);
        }
