    ...
    class OrderController extends MyRestController {
        ...
        {                 
            try {                
                ...
                $sum = $this->getSum($this->user->id, $service_type, $area_id);
                ...
                if(isset($redeem_type)) {
                    ...
                    $discount = $this->getDiscount($redeem_type, $this->user->id, $service_type, $coupon_code);
                    $order['discount_amount'] = $discount['discount_amount'];

                    $order['coupon_point_apply'] = $redeem_type;                

                    $point = $this->getPoint($this->user->id, $service_type);                
        ...
        {                 
            try {                
                ...
                $this->response(array(
                    "code"=>RESULT_SUCCESS,    
                    "resource"=>$this->getSum($this->user->id, $service_type, $area_id)
                    ), REST_Controller::HTTP_OK);
        ...
        {                 
            try {                
                ...
                $this->response(array(
                    "code"=>RESULT_SUCCESS,    
                    "resource"=>$this->getDiscount($redeem_type, $this->user->id, $service_type, $coupon_code)
                    ), REST_Controller::HTTP_OK);
        ...
        {                 
            try {                
                ...
                $this->response(array(
                    "code"=>RESULT_SUCCESS,    
                    "resource"=>$this->getPoint($this->user->id, $service_type)
                    ), REST_Controller::HTTP_OK);
        ...
        public function getPoint($user_id, $service_type) {                       
            ...
            foreach($carts as $cart) {
                ...
                if(!isset($subtotal_by_location[$cart->location_id])) {
                    $subtotal_by_location[$cart->location_id] = $subtotal;
                } else {
                    $subtotal_by_location[$cart->location_id] += $subtotal;
                }
                ...
                $loyalty_point = $this->LoyaltyPointModel->findOne(array(
                    "restro_id"     => $cart->restro_id,
                    "location_id"   => $cart->location_id,
                    "service_id"    => $service_type
                ));

                $loyalty_points[$cart->location_id] = $loyalty_point;
            }
            ...
            $loyalty_discount_amount = 0; $loyalty_used_points = 0; 

            foreach($subtotal_by_location as $location_id=>$subtotal) {
                $loyalty_point = $loyalty_points[$location_id];

                $discount = 0; $used_points = 0; 

                if($loyalty_point) {
                    if($user_loyalty_points >= $loyalty_point->from1) {
                        $discount = $loyalty_point->discount1;
                        $used_points = $loyalty_point->from1;
                    }
                    if($user_loyalty_points >= $loyalty_point->from2 && $discount < $loyalty_point->discount2) {
                        $discount = $loyalty_point->discount2;
                        $used_points = $loyalty_point->from2;
                    }
                    if($user_loyalty_points >= $loyalty_point->from3 && $discount < $loyalty_point->discount3) {
                        $discount = $loyalty_point->discount3;
                        $used_points = $loyalty_point->from3;
                    }
                }
                ...
                $loyalty_used_points += $used_points;
            }
        ...
        public function getSum($user_id, $service_type, $area_id=null) {                       
            $carts = $this->CartModel->find($service_type, array(
                'user_id'   => $user_id
            ));       
            ...
            $total_amount = 0; $charge_amount_by_location = array();
            foreach($carts as $cart) {
                $total_amount += $cart->price * $cart->quantity;

                if(($service_type==1 || $service_type==2) && $area_id) { // service type is "DELIVERY" or "CATERING"      
                    if(!isset($charge_amount_by_location[$cart->location_id])) {
                        $charge_amount_by_location[$cart->location_id] = $this->RestroCityAreaModel->getCharge($cart->restro_id, $cart->location_id, $area_id, $service_type);
                    }
                }
            }
            ...
            return array(
                ...
                'charge_amount'=>array_sum($charge_amount_by_location)
            );
        }

        public function getDiscount($redeem_type, $user_id, $service_type, $coupon_code) {
            $carts = $this->CartModel->find($service_type, array(
                "user_id"   => $user_id
            ));     
            ...
            $total_amount = 0; 
            $subtotal_by_location = array();
            $loyalty_points_by_locaiton = array();
            ...
            foreach($carts as $cart) {
                ...
                $loyalty_points_by_locaiton[$cart->location_id] = $loyalty_point;
            }
            ...
            if($redeem_type == 1) { // Coupon
                ...
                if($coupon->from_date != '')
                {
                    ...
                    {                                 
                        return array('discount_amount'=>($subtotal_by_location[$coupon->location_id] * $coupon->discount) / 100);
                    } else {
                ...
                foreach($subtotal_by_location as $location_id=>$subtotal) {
                    $loyalty_point = $loyalty_points_by_locaiton[$location_id];
                ...
                return array('discount_amount'=>$loyalty_discount_amount);
            } else if($redeem_type == 3) {  // Mataam Point
                // Calculate Mataam Point
